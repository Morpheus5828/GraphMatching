...
class Barycenter:
    ...
    def fgw_barycenters(
        self, N, Ys, Cs,
        ps=None, lambdas=None, alpha=0.5, fixed_structure=False,
        ...
        Cs = utils.list_to_array(*Cs)
        Ys = utils.list_to_array(*Ys)
        arr = [*Cs, *Ys]
        if ps is not None:
            arr += utils.list_to_array(*ps)
        else:
            ps = [utils.unif(C.shape[0], type_as=C) for C in Cs]
        if p is not None:
            arr.append(utils.list_to_array(p))
        else:
            p = utils.unif(N, type_as=Cs[0])

        nx = utils.get_backend(*arr)
        ...
        if fixed_structure:
            if init_C is None:
                raise utils.UndefinedParameter('If C is fixed it must be initialized')
            else:
                C = init_C
        else:
            if init_C is None:
        ...
        if fixed_features:
            if init_X is None:
                raise utils.UndefinedParameter('If X is fixed it must be initialized')
            else:
                X = init_X
        else:
            if init_X is None:
        ...
        Ms = [utils.dist(X, Ys[s]) for s in range(len(Ys))]
        ...
        while ((err_feature > tol or err_structure > tol or err_rel_loss > tol) and cpt < max_iter):
            ...
            if not fixed_features:
                ...
                X = utils.update_feature_matrix(lambdas, Ys_temp, T, p, nx).T
                Ms = [utils.dist(X, Ys[s]) for s in range(len(Ys))]

            if not fixed_structure:
                if loss_fun == 'square_loss':
                    C = utils.update_square_loss(p, lambdas, T, Cs, nx)

                elif loss_fun == 'kl_loss':
                    C = utils.update_kl_loss(p, lambdas, T, Cs, nx)
